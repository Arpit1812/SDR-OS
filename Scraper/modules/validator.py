...
def validate_icp() -> None | ValueError | TypeError:
    '''
    Validates all variables in the `/config/icp.py` file.
    '''
    global __validation_file_path
    __validation_file_path = "config/icp.py"

    check_string(lead_extraction_mode, "lead_extraction_mode", ["apply_only", "extractor_only", "hybrid"])
    check_string(icp_match_mode, "icp_match_mode", ["strict", "score"])
    check_int(icp_score_threshold, "icp_score_threshold", 0)
    if icp_score_threshold > 100:
        raise ValueError('The variable "icp_score_threshold" in "config/icp.py" expects an Integer between 0 and 100.')

    check_string(icp_leads_file_name, "icp_leads_file_name", min_length=1)
    check_string(icp_search_location, "icp_search_location")
    check_string(icp_sort_by, "icp_sort_by", ["", "Most recent", "Most relevant"])
    check_string(icp_date_posted, "icp_date_posted", ["", "Any time", "Past month", "Past week", "Past 24 hours"])
    check_string(icp_salary, "icp_salary")
    check_boolean(icp_easy_apply_only, "icp_easy_apply_only")
    check_list(icp_experience_level, "icp_experience_level", ["Internship", "Entry level", "Associate", "Mid-Senior level", "Director", "Executive"])
    check_list(icp_job_type, "icp_job_type", ["Full-time", "Part-time", "Contract", "Temporary", "Volunteer", "Internship", "Other"])
    check_list(icp_on_site, "icp_on_site", ["On-site", "Remote", "Hybrid"])
    check_list(icp_search_terms, "icp_search_terms")
    check_list(icp_role_keywords, "icp_role_keywords", min_length=1)
    check_list(icp_company_size_targets, "icp_company_size_targets", min_length=1)
    check_list(icp_industry_keywords, "icp_industry_keywords", min_length=1)
    check_list(icp_non_technical_signals, "icp_non_technical_signals")
    check_list(icp_technical_exclusion_signals, "icp_technical_exclusion_signals")
    check_boolean(icp_dedupe_by_job_id, "icp_dedupe_by_job_id")
    company_size_score_mode_flag = globals().get(
        "icp_require_company_size_in_score_mode",
        globals().get("icp_require_company_size_in_scro_mode", False)
    )
    industry_score_mode_flag = globals().get("icp_require_industry_in_score_mode", False)
    check_boolean(company_size_score_mode_flag, "icp_require_company_size_in_score_mode")
    check_boolean(industry_score_mode_flag, "icp_require_industry_in_score_mode")
    check_boolean(icp_title_pre_filter, "icp_title_pre_filter")


from config.icp_posts import *
def validate_icp_posts() -> None | ValueError | TypeError:
    '''
    Validates all variables in the `/config/icp_posts.py` file.
    '''
    global __validation_file_path
    __validation_file_path = "config/icp_posts.py"

    check_list(icp_posts_search_terms, "icp_posts_search_terms")
    check_int(icp_posts_max_pages, "icp_posts_max_pages", 1)
    check_int(icp_posts_scroll_rounds, "icp_posts_scroll_rounds", 1)
    check_int(icp_posts_retry_limit, "icp_posts_retry_limit", 1)
    check_int(icp_posts_sleep_ms, "icp_posts_sleep_ms", 100)
    check_string(icp_posts_detail_open_mode, "icp_posts_detail_open_mode", ["off", "same_tab", "new_tab"])
    check_string(icp_posts_leads_file_name, "icp_posts_leads_file_name", min_length=1)
    check_int(icp_posts_snippet_max_length, "icp_posts_snippet_max_length", 200)
