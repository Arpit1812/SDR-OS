...
def get_default_temp_profile() -> str:
    """Returns a plain file-system path for a temporary Chrome profile (no CLI flag prefix)."""
...
def print_lg(*msgs: str | dict, end: str = "\n", pretty: bool = False, flush: bool = False, from_critical: bool = False) -> None:
    ...
    for message in msgs:
        try:
            ...
            pass
        # Try writing to log file with retry
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    # Fallback: write to a timestamped alternative log file
                    try:
                        alt_path = __logs_file_path.replace("log.txt", f"log_{datetime.now().strftime('%H%M%S')}.txt")
                        with open(alt_path, 'a+', encoding="utf-8") as file:
                            file.write(str(message) + end)
                    except Exception:
                        pass  # Silently skip if all logging fails
            except Exception:
                break  # Non-permission errors, skip logging
#>
